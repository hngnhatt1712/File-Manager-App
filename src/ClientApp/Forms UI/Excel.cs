...
{
    public partial class Excel : UserControl, ISearchable, ISortable
    {
        private FileTransferClient _client;
        private List<FileMetadata> _cachedFiles = new List<FileMetadata>();
        public Excel(FileTransferClient client)
        {
            InitializeComponent();
            _client = client;
        }

        private async void Excel_Load(object sender, EventArgs e)
        {
            fileList1.SetClient(_client);
            fileList1.AllowedExtensions = new[] { ".xls" };
